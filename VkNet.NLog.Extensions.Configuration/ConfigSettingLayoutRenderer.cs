...
namespace VkNet.NLog.Extensions.Configuration
{
	/// <summary>
	...
	public class ConfigSettingLayoutRenderer: LayoutRenderer
    {
        ...
        private static void AddFileConfiguration(ConfigurationBuilder configBuilder, string fullPath)
        {
            if (System.IO.File.Exists(fullPath))
            {
                ...
                {
                    InternalLogger.Info("configSetting - Skipping FileName with unknown file-extension: {0}", fullPath);
                }
            }
            else
            {
                InternalLogger.Info("configSetting - Skipping FileName as file doesnt't exists: {0}", fullPath);
            }
        }
    }
}